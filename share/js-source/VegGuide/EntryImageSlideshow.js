...
VegGuide.EntryImageSlideshow.prototype._showImage = function (imageNumber) {
    ...
    img.src    = image.files[2].uri;
    img.alt    = "";
    img.height = image.files[2].height;
    img.width  = image.files[2].width;
...
VegGuide.EntryImageSlideshow.prototype._getImage = function (imageNumber) {
    if ( ! this.images[ imageNumber - 1 ] ) {
        ...
        image.margin_top  = ( ( container_height - image.files[2].height ) / 2 ) + "px";
        image.margin_left = ( ( container_width - image.files[2].width ) / 2 ) + "px";
