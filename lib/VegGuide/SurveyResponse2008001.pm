...
use VegGuide::Schema;
use VegGuide::AlzaboWrapper ( table => VegGuide::Schema->Schema()->SurveyResponse2008001_t() );
...
sub VisitFrequencies
{
    ...
    my %freq;
    for my $row ( $class->table()->function
                      ( select   => [ $class->table()->visit_frequency_c(),
...
sub Activities
{
    ...
    my ($row) =
        $class->table()->function
            ( select   => [ map { $schema->sqlmaker()->SUM($_) }
    ...
    return
        $class->table()->function
            ( select   => $class->table()->column('other_sites_other'),
              where    => [ $class->table()->column('other_sites_other'), '!=', undef ],
            );
}
...
sub Improvements
{
    ...
    my $handle =
        $class->table()->select
            ( select   => [ $class->table()->columns( 'email_address', 'improvements' ) ],
