...
sub convert_empty_strings
{
    ...
    foreach my $col ( grep { $tri_state_cols->{$_} || $char_cols->{$_} || $num_cols->{$_} } keys %$vals )
    {
        if ( exists $vals->{$col} && defined $vals->{$col} && ! length $vals->{$col} )
        {
            $vals->{$col} =
...
{
    my $spec = { text        => { type => SCALAR | UNDEF },
    ...
    sub text_to_html
    {
        ...
        if ( $p{first_class} )
        {
            if ( $p{class} )
            {
                $html =~ s/<p class="\Q$p{class}">/<p class="$p{class} $p{first_class}">/
            }
            else
            {
...
sub _format_header
{
    ...
    return "<h$level>", Text::WikiFormat::format_line($_[3], @_[-2, -1]), "</h$level>\n";
}
