...
use VegGuide::Validate qw( validate validate_with validate_pos SCALAR UNDEF BOOLEAN LOCATION_TYPE );

use VegGuide::Schema;
use VegGuide::AlzaboWrapper
    ( table => VegGuide::Schema->Schema()->User_t() );
...
sub _new_row
{
    ...
    {
        push @where,
            [ $schema->User_t->real_name_c, '=', $p{real_name} ];
    }
...
sub _validate_data
{
    ...
    if ( defined $data->{real_name} )
    {
        for my $re ( qr{multipart/alternative}i,
                     qr{content-type:}i,
    ...
    if ( ! $is_update || defined $data->{password} )
    {
        ...
        unless ( string_is_empty( $data->{password} ) )
        {
            push @errors, "The two passwords must be identical."
                unless $data->{password2} && $data->{password} eq $data->{password2};
...
sub is_location_owner
{
    ...
    return
        $schema->LocationOwner_t->function
            ( select => 1,
              where =>
...
sub vendor_count
{
    ...
    push @where, [ $schema->Vendor_t->location_id_c, '=', $p{location}->location_id() ]
        if $p{location};
    ...
    return
        $schema->row_count
            ( join   =>
              ...
              where  =>
              [ [ $schema->VendorRating_t()->user_id_c(), '=', $self->user_id() ],
                [ $schema->Vendor_t()->vendor_id_c(), 'NOT IN', $reviewed_vendor_subselect ],
              ]
            );
}
...
sub _reviewed_vendor_subselect
{
    ...
    $sql->select( $schema->VendorComment_t()->vendor_id_c() )
        ...
        ->where( $schema->VendorComment_t()->user_id_c(), '=', $self->user_id() );
...
sub average_rating
{
    ...
    return
        $schema->VendorRating_t()->function
            ( select => $schema->sqlmaker()->AVG( $schema->VendorRating_t()->rating_c() ),
              where  =>
              [ $schema->VendorRating_t()->user_id_c(), '=', $self->user_id() ],
            );
}
...
sub is_subscribed_to_location
{
    ...
    return
        $schema->UserLocationSubscription_t->function
            ( select => 1,
              where =>
...
sub send_subscription_email
{
    ...
    my $new_vendors =
        $self->new_entries_for_subscription(@_);

    return unless $new_vendors;

    my %params = ( vendors1 => $new_vendors,
...
sub new_entries_for_subscription
{
    ...
    {
        push @location_ids,
            $location->location_id, $location->descendant_ids;
    }
...
sub viewable_suggestions
{
    ...
    my %p = validate( @_,
                      { order_by   =>
                        { type => SCALAR, default => 'creation_datetime' },
                        sort_order =>
    ...
    my @order_by;
    @order_by =
        ( $schema->VendorSuggestion_t->creation_datetime_c, $p{sort_order} );

    if ( $self->is_admin )
    {
        return
            $self->cursor
                ( $schema->VendorSuggestion_t->all_rows
                      ( order_by => \@order_by )
            );
    }
    else
...
sub _viewable_suggestion_params
{
    ...
    return
        ( join => [ [ $schema->tables( 'Vendor', 'VendorSuggestion' ) ],
                    [ $schema->tables( 'Vendor', 'Location' ) ],
                    [ left_outer_join =>
...
sub activity_log_count
{
    ...
    return
        $schema->UserActivityLog_t()->row_count
            ( where =>
              [ $schema->UserActivityLog_t()->user_id_c(), '=', $self->user_id() ],
            );
}
...
sub activity_logs
{
    ...
    return
        $self->cursor( $self->row_object->activity_logs
                           ( order_by =>
...
{
    my $types = VegGuide::Schema->Connect()->UserActivityLogType_t()->all_rows();
    my %types;

    while ( my $t = $types->next )
    {
        $types{ $t->select('type') } = $t->select('user_activity_log_type_id');
    }
...
sub _WhereClauseForSearch
{
    ...
    if ( $p{real_name} )
    {
        $where{where} = [ $user_t->real_name_c(),
                          'LIKE', '%' . $p{real_name} . '%' ];
    }
...
sub CountForDateSpan
{
    ...
    return
        $class->table->function
            ( select => VegGuide::Schema->Connect()->sqlmaker->COUNT( $class->table->user_id_c ),
              where  =>
...
{
    my $guest_row =
        VegGuide::Schema->Schema()->User_t()->potential_row
            ( values => { user_id          => 0,
