...
use VegGuide::AlzaboWrapper
    ( table => VegGuide::Schema->Schema->Team_t );
...
sub _validate_data
{
    ...
    data_validation_error
        error => "One or more data validation errors", errors => \@errors
            if @errors;
...
sub member_count
{
    ...
    return
        $schema->User_t->row_count
            ( where =>
              [ $schema->User_t->team_id_c, '=', $self->team_id ],
            );
}
