...
use VegGuide::Validate qw( validate validate_with UNDEF SCALAR ARRAYREF BOOLEAN );
...

{
    ...
    DateTime::Locale->register( id => en_Ghana =>
                                en_language  => 'English',
                                en_territory => 'Ghana',
...

{
    ...
    DateTime::Locale->register( id => en_Kenya =>
                                en_language  => 'English',
                                en_territory => 'Kenya',
                              );
}
...
sub _PreloadLocales
{
    ...
    my $locales =
        $schema->Locale_t->select
            ( select =>
              $schema->sqlmaker->DISTINCT
                  ( $schema->Locale_t->locale_code_c ),
            );
...
sub _get_encodings
{
    ...
    $self->{encodings} =
        { map { $_ => 1 } map { $_->select('encoding_name') } $self->row_object->encodings->all_rows };
}
    ...
    eval
    {
        ...
        foreach my $name (@encodings)
        {
            $schema->LocaleEncoding_t->insert
                ( values => { locale_id => $self->locale_id,
                              encoding_name => $name },
                );
        }
...
sub All
{
    ...
    my @order_by =
        ( $schema->Locale_t->locale_code_c, $p{sort_order} );
...

{
    ...
    my @encodings =
        ( sort _sort_encodings
          grep { ! ( $exclude{$_} || /$exclude_re/ ) } Encode->encodings()
        );
...
use VegGuide::Schema;
use VegGuide::AlzaboWrapper ( table => VegGuide::Schema->Schema->AddressFormat_t );
