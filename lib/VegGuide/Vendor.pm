...
sub create
{
    ...
    my $organization_category_id = VegGuide::Category->Organization->category_id;
    ...
    eval
    {
        ...
        {
            foreach my $id ( ref $payment_option_ids ?
                             @$payment_option_ids : $payment_option_ids )
            {
        ...
        if ( $attribute_ids )
        {
            foreach my $id ( ref $attribute_ids ?
        ...
        $user->insert_activity_log( type      => 'add vendor',
                                    vendor_id => $vendor->vendor_id,
                                  );
...
{
    my $Parser = DateTime::Format::Strptime->new( pattern => '%Y-%m-%d', on_error => 'croak' );
    ...
    sub update
    {
        ...
        $p{localized_neighborhood} = $p{new_localized_neighborhood}
            if defined $p{new_localized_neighborhood} && length $p{new_localized_neighborhood};
        ...
        eval
        {
            if ( $p{category_id} )
            {
                $_->delete
                    foreach
                        $schema->VendorCategory_t->rows_where
                            ( where =>
            ...
            if ( $p{payment_option_id} )
            {
                $_->delete
                    foreach
                        $schema->VendorPaymentOption_t->rows_where
                            ( where =>
            ...
            if ( $p{cuisine_id} )
            {
                $_->delete
                    foreach
                        $schema->VendorCuisine_t->rows_where
                            ( where =>
            ...
            if ( $p{attribute_id} )
            {
                $_->delete
                    foreach
                        $schema->VendorAttribute_t->rows_where
                            ( where =>
            ...
            $user->insert_activity_log( type      => 'update vendor',
                                        vendor_id => $self->vendor_id,
                                      )
...
sub save_core_suggestion
{
    ...
    $p{suggestion}{neighborhood} = $p{suggestion}{new_neighborhood}
        if defined $p{suggestion}{new_neighborhood} && length $p{suggestion}{new_neighborhood};

    delete $p{suggestion}{new_neighborhood};

    $p{suggestion}{localized_neighborhood} = $p{suggestion}{new_localized_neighborhood}
        if defined $p{suggestion}{new_localized_neighborhood} && length $p{suggestion}{new_localized_neighborhood};

    delete $p{suggestion}{new_localized_neighborhood};

    my %suggestion;

    $p{suggestion}{home_page} = VegGuide::Util::normalize_uri( $p{suggestion}{home_page} )
                    ...
                    grep { ! $_->is_primary_key } @Columns )
    {
        ...
        $suggestion{$c} = $p{suggestion}{$c}
            unless ( ( defined $p{suggestion}{$c} && defined $current &&
    ...
    foreach my $key ( qw( category_id payment_option_id cuisine_id attribute_id ) )
    {
        ...
        my $meth = $key . 's';
        unless ( VegGuide::Util::arrays_match( $ids, [ sort $self->$meth() ] ) )
        {
    ...
    {
        $suggestion =
            $schema->VendorSuggestion_t->insert
                ( values =>
                  { type => 'core',
        ...
        $p{user}->insert_activity_log( type      => 'suggest a change',
                                       vendor_id => $self->vendor_id,
                                       comment   => 'core data',
                                     );
        ...
        [ $self->cursor
              ( $schema->join
                    ( select => $schema->Attribute_t,
                      ...
                      where  =>
                      [ $schema->VendorAttribute_t->vendor_id_c, '=', $self->vendor_id ],
...

{
    ...
    my %tri_state_cols =
        map { $_->name => 1 }
        grep { $_->type eq 'TINYINT' && $_->length && $_->length == 1 && $_->nullable }
        @Columns;
...
sub _cuisine_ancestors
{
    ...
    my @ids;
    while ( $id =
            $schema->Cuisine_t->function
                ( select => $schema->Cuisine_t->parent_cuisine_id_c,
                  where  =>
                  [ $schema->Cuisine_t->cuisine_id_c, '=', $id ]
                )
          )
    {
        push @ids, $id;
    ...
    return
        ( map { VegGuide::PaymentOption->new( object => $_ ) }
          $self->row_object->payment_options
              ( where    =>
                [ $schema->PaymentOption_t->name_c, '!=', 'Check' ],
                order_by =>
...
sub cuisines
{
    ...
    return
        map { VegGuide::Cuisine->new( cuisine_id => $_ ) }
            $self->cuisine_ids;
}
...
sub add_or_update_comment
{
    ...
    if ( $comment = $self->comment_by_user( $p{user} ) )
    {
        ...
        if ( $p{user}->user_id == $p{calling_user}->user_id )
        {
            $p{user}->insert_activity_log( type      => 'update review',
                                           vendor_id => $self->vendor_id,
                                         );
        }
    }
    else
    {
        ...
        $p{user}->insert_activity_log( type      => 'add review',
                                       vendor_id => $self->vendor_id,
                                     );
    }
...
sub rating_from_user
{
    ...
    my $rating =
        $schema->VendorRating_t->function
...
BEGIN
{
    Alzabo::SQLMaker::make_function( function => 'WEIGHTED_RATING',
                                     min      => 3,
                                     max      => 3,
                                     groups   => [ 'udf' ],
                                   );
...
sub WeightedRating
{
    ...
    return
        sprintf( '%.1f',
                 ( $count / ( $count + $WeightedRatingMinCount ) )
                 * $average
...
sub ratings_without_review_count
{
    ...
    push @where, [ $schema->VendorRating_t->user_id_c, 'NOT IN',
...
sub _ratings_without_reviews_subselect
{
    ...
    return
        $schema->sqlmaker()
            ...
            ->where( $schema->VendorComment_t()->vendor_id_c(), '=', $self->vendor_id() );
}
...
sub price_range
{
    ...
    {
        return VegGuide::PriceRange->new( price_range_id => $self->price_range_id );
    }
    else
    {
...
sub hour_sets_by_day
{
    ...
    {
        my %times = ( open_minute  => $set->select('open_minute'),
    ...
    for my $day ( 0..$#raw )
    {
        ...
        elsif ( $raw[$day] )
        {
            for my $set ( @{ $raw[$day] } )
            {
                ...
                my $open  = VegGuide::Vendor->MinutesToTime( $set->{open_minute} );
                my $close = VegGuide::Vendor->MinutesToTime( $set->{close_minute} );
...
sub has_hours_info
{
    ...
    return
        $schema->VendorHours_t->row_count
            ( where =>
...
sub hours_are_complete
{
    ...
    my $days_known =
        $schema->VendorHours_t->function
            ( select =>
              $schema->sqlmaker->COUNT( $schema->sqlmaker->DISTINCT( $schema->VendorHours_t->day_c ) ),
              where  =>
              [ $schema->VendorHours_t->vendor_id_c, '=', $self->vendor_id ],
            );
...
sub hours_as_descriptions
{
    ...
    foreach my $day ( 0..6 )
    {
        ...
        {
            if ( $p{sets}->[$day][0]{open_minute} eq $p{sets}->[$day][0]{close_minute} )
            {
             ...
             ! VegGuide::Util::arrays_match( \@current_hours, \@last_hours ) )
        {
            ...
            {
                $day_desc = "$days_abbr[$last_day_changed] - $days_abbr[$day - 1]";
            }
            else
            {
            ...
            push @descriptions, { days  => $day_desc,
                                  hours => [ @last_hours ],
                                };
...

{
    ...
    sub _format_time
    {
        ...
        my $time = $dt->strftime( $dt->minute() ? $FormatWithMinutes : $FormatWithoutMinutes );
...
sub HoursRangeToMinutes
{
    ...
    return { open_minute  => ( $dt1->hour() * 60 ) + $dt1->minute(),
...

{
    ...
    sub _parse_time
    {
        ...
        {
            my $ampm;
            ( $h, $m, $ampm ) = $time =~ m{ (\d\d?) (?: [:./] (\d\d) )? \s* (a|p)? }ixsm;
            $m ||= 0;
...
sub replace_hours
{
    ...
    eval
    {
        ...
        $self->SUPER::update( last_modified_datetime => $schema->sqlmaker->NOW() );
...
sub _address_pieces_standard
{
    return
        ( grep { defined }
          $_[0]->address1, $_[0]->address2, $_[0]->city_region_postal_code
        );
}
...
sub _address_pieces_Hungarian
{
    return
        ( join ', ',
          grep { defined }
          $_[0]->postal_code, $_[0]->city, $_[0]->region, $_[0]->address1, $_[0]->address2
        );
}
...
{
    return 1
        if grep { defined && length } $_[0]->_localized_address_pieces_standard();
}
...
sub _localized_address_pieces_standard
{
    return
        ( grep { defined }
          $_[0]->localized_address1, $_[0]->localized_address2, $_[0]->localized_city_region_postal_code
        );
}
...
sub _localized_address_pieces_Hungarian
{
    return
        ( join ', ',
          ...
          $_[0]->localized_region, $_[0]->localized_address1, $_[0]->localized_address2
        );
}
...
sub localized_city_region_postal_code
{
    ...
    my ($city, $region, $postal_code) =
        ( $self->localized_city, $self->localized_region, $self->postal_code );
...
sub map_uri
{
    ...
    if ( $geocoder && ! string_is_empty( $self->canonical_address() ) )
    {
        return uri( scheme => 'http',
                    ...
                    path   => '/maps',
                    query  => { q => Encode::encode( 'utf8', $self->canonical_address() ) },
                  );
    }
    else
    {
        my $map_link =
            WebService::StreetMapLink->new( country     => $self->location()->country(),
...
sub update_geocode_info
{
    ...
    my %p;
    for my $meth ( qw( address1 localized_address1
                       city localized_city
    ...
    $self->SUPER::update( latitude          => $result->latitude(),
...
sub images
{
    ...
    $self->{images} = [ sort { $a->display_order() <=> $b->display_order() }
...
my $OrganizationCategoryID =
    VegGuide::Category->Organization ? VegGuide::Category->Organization->category_id : 0;
sub is_organization
{
    my $self = shift;
    ...
    return
        ( @cat == 1 &&
          $cat[0]->category_id == $OrganizationCategoryID ?
          1 :
          0 );
}

my $RestaurantCategoryID =
    VegGuide::Category->Restaurant ? VegGuide::Category->Restaurant->category_id : 0;
sub is_restaurant
{
    my $self = shift;

    return grep { $_->category_id == $RestaurantCategoryID } $self->categories;
}
...
my $BarCategoryID = VegGuide::Category->Bar ? VegGuide::Category->Bar->category_id : 0;
my $CoffeeTeaJuiceCategoryID =
    VegGuide::Category->CoffeeTeaJuice ? VegGuide::Category->CoffeeTeaJuice->category_id : 0;
...
sub location
{
    ...
    return $self->{location}
        if $self->{location} && $self->location_id == $self->{location}->location_id;

    return
        $self->{location} = VegGuide::Location->new( location_id => $self->location_id );
}
...
sub rest_data
{
    ...
    my %rest = map { $_ => $self->$_() } map { $_->name() } $self->table()->columns();
...
sub VendorCount
{
    ...
    unless ( $p{join} && UNIVERSAL::isa( $p{join}, 'ARRAY' ) && @{ $p{join} } )
    {
...
BEGIN
{
    Alzabo::SQLMaker::make_function( function => 'GREAT_CIRCLE_DISTANCE',
                                     min      => 5,
                                     max      => 5,
                                     groups   => [ 'udf' ],
                                   );
...
sub VendorsWhere
{
    ...
    my %p = validate( @_,
                      { order_by => { default => 'name' },
                        ...
                        join   => { optional => 1 },
                        lat_long => { optional => 1 },
                        unit     => { type => SCALAR, regex => qr/^(?:mile|km)$/, optional => 1 },
                      },
                    );
    ...
    elsif ( lc $p{order_by} eq 'rating' )
    {
        ...
        return
            VegGuide::Cursor::VendorByAggregate->new
                ( cursor =>
                  $schema->select
                      ( select =>
                        [ $schema->Vendor_t->vendor_id_c, $wr, $count ],
                        join => \@join,
                        %where,
                        group_by =>
                        [ $schema->Vendor_t->vendor_id_c, ],
        ...
        return
            VegGuide::Cursor::VendorByAggregate->new
                ( cursor =>
                  $schema->select
                      ( select =>
                        [ $schema->Vendor_t()->vendor_id_c(), $distance ],
                        join => \@join,
                        %where,
                        order_by =>
                        [ $distance,
...
sub VendorIdsWithMinimumRating
{
    ...
    {
        @where =
            $class->LatitudeLongitudeMinMaxWhere( $p{latitude_longitude_min_max} );
    }
    elsif ( $p{name} )
    {
        @where =
            $class->NameWhere( $p{name} );
    }
    else
    {
...
sub LatitudeLongitudeMinMaxWhere
{
    ...
    return
        ( [ $class->table->latitude_c,  'BETWEEN', $lat_long->[0], $lat_long->[1] ],
          [ $class->table->longitude_c, 'BETWEEN', $lat_long->[2], $lat_long->[3] ],
        );
...
sub RecentlyReviewedCount
{
	...
	$schema->row_count
	    ( join  =>
	      [ $schema->tables( 'Vendor', 'VendorComment', 'User' ) ],
	      where =>
...
sub RecentlyReviewed
{
    ...
    if ( $p{location_ids} )
    {
        my @ids = ref $p{location_ids} ? @{ $p{location_ids} } : $p{location_ids};

        push @where,
            [ $class->table->location_id_c, 'IN', @ids ];
    }
...
sub EarliestCreationDate
{
    ...
    return DateTime::Format::MySQL->parse_datetime($date)->truncate( to => 'day' );
}
...
use VegGuide::Schema;
use VegGuide::AlzaboWrapper ( table => VegGuide::Schema->Schema->VendorRating_t );
...
use VegGuide::Schema;
use VegGuide::AlzaboWrapper ( table => VegGuide::Schema->Schema->VendorSuggestion_t,
...
sub text_changes
{
    ...
    my %changes;
    foreach my $col ( sort map { $_->name } grep { $_->is_character || $_->is_blob }
                      $schema->Vendor_t->columns )
    {
...
sub payment_option_changes
{
    ...
    return $self->_add_delete_map( 'payment_option_id', 'VegGuide::PaymentOption' );
}
...
sub _add_delete_map
{
    ...
    return
        ( add    => [ map { $class->new( $key => $_ ) } grep { ! $current{$_} } keys %new ],
          remove => [ map { $class->new( $key => $_ ) } grep { ! $new{$_} } keys %current ],
        );
}
...
sub accept
{
    ...
    $self->user->insert_activity_log( type      => 'suggestion accepted',
                                      vendor_id => $self->vendor_id,
                                      comment   => $log_comment,
                                    );
...
sub reject
{
    ...
    $self->user->insert_activity_log( type      => 'suggestion rejected',
                                      vendor_id => $self->vendor_id,
                                      comment   => $log_comment,
                                    );
...
use VegGuide::Schema;
use VegGuide::AlzaboWrapper ( table => VegGuide::Schema->Schema->PaymentOption_t );

use VegGuide::Validate
    qw( validate_with SCALAR );
...
sub _new_row
{
	...
	my @where;
	push @where,
	    [ $schema->PaymentOption_t->name_c, '=', $p{name} ];
