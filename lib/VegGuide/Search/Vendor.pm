...
sub _process_sql_query {
    ...
    for my $k (qw( category_id cuisine_id attribute_id )) {
        ...
        $self->{$k} = [ grep { defined && length && $class->IsValidId($_) }
                ref $self->{$k} ? @{ $self->{$k} } : $self->{$k} ];
    }

    $self->{neighborhood}
        = [ grep { defined && length }
            ref $self->{neighborhood}
        ? @{ $self->{neighborhood} }
        : $self->{neighborhood} ];
    ...
    if ( $self->{allow_closed} ) {
        ...
        push @{ $self->{where} },
            [ VegGuide::Schema->Schema()->Vendor_t->close_date_c, '=',
            undef ];
    }
    ...
    $self->$_()
        for grep { $self->can($_) } map { ("_$_") } $self->SearchKeys();
}
...
sub _cuisine_id {
    ...
    my @c = map { VegGuide::Cuisine->new( cuisine_id => $_ ) }
        @{ $self->{cuisine_id} };

    my %all_ids
        = map { $_ => 1 } map { ( $_->descendant_ids, $_->cuisine_id ) } @c;
    ...
    if ( @names == 1 ) {
        $self->{descriptions}{cuisine_id}{long}
            = " which serve $names[0] food.";
    }
    else {
...
sub _neighborhood {
    ...
    if ( @n == 1 ) {
        $self->{descriptions}{neighborhood}{long}
            = " in the $n[0] neighborhood.";
    }
    elsif ( @n == 2 ) {
        $self->{descriptions}{neighborhood}{long}
            = " in the $n[0] or $n[1] neighborhoods.";
    }
    else {
...
sub _city {
    ...
    push @{ $self->{where} },
        [ VegGuide::Schema->Schema()->Vendor_t->city_c, 'LIKE',
        "%$self->{city}%" ];

    $self->{descriptions}{city}{long}
        = qq| in a city with a name like "$self->{city}".|;
...
sub _veg_level {
    ...
    if ( $self->{veg_level} == 2 ) {
        ...
        push @{ $self->{where} },
            [ VegGuide::Schema->Schema()->Vendor_t->veg_level_c, '>=',
            $self->{veg_level} ];
    }

    $self->{descriptions}{veg_level}{long} = 'which are at least '
...
sub _accepts_reservations {
    ...
    push @{ $self->{where} },
        [ VegGuide::Schema->Schema()->Vendor_t->accepts_reservations_c, '=',
        1 ];

    $self->{descriptions}{accepts_reservations}{long}
        = 'which accept reservations';
...
sub _is_wheelchair_accessible {
    ...
    push @{ $self->{where} },
        [ VegGuide::Schema->Schema()->Vendor_t->is_wheelchair_accessible_c,
        '=', 1 ];

    $self->{descriptions}{is_wheelchair_accessible}{long}
...
sub _price_range_id {
    ...
    my $range = VegGuide::PriceRange->new(
...
sub _attribute_id {
    ...
    foreach my $id ( @{ $self->{attribute_id} } ) {
        ...
        push @{ $self->{join} },
            [ VegGuide::Schema->Schema()->Vendor_t, $alias ];
...
sub _rating {
    ...
    $self->{descriptions}{rating}{long}
        = " with a rating of at least $self->{rating}.";
...
sub _days {
    ...
    $self->{descriptions}{days}{long}
        = "which were added within the last $days_text.";
