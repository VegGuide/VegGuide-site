...
sub _open_for {
    ...
    push @{ $self->{where} },
        VegGuide::Vendor->is_open_where_clause( $dt, $vh1 );

    my $round_min = 15 - ( $dt->minute() % 15 );
    $dt->add( minutes => $self->{open_for} + $round_min );

    push @{ $self->{where} },
        VegGuide::Vendor->is_open_where_clause( $dt, $vh2 );
...
sub count {
    return $_[0]->{location}->active_vendor_count(
        where => $_[0]->{where},
        join  => $_[0]->{join},
    );
}
...
sub map_uri {
    ...
    return region_uri( $self->_uri_base_params('map') );
}

sub printable_uri {
    my $self = shift;

    return region_uri( $self->_uri_base_params('printable') );
}
