...
sub GrowthOverTime {
    ...
    for my $d (@dates) {
        ...
        my $u_count = VegGuide::User->CountForDateSpan(
            start_date => $start,
            end_date   => $end,
        );
        ...
        my $v_count = VegGuide::Vendor->CountForDateSpan(
            start_date => $start,
            end_date   => $end,
        );
    ...
    my $yaxis = Chart::OFC::YAxis->new(
        axis_label  => 'Count',
        label_steps => 500,
        max         => nhimult( 500, $biggest + 1 ),
    );

    my $user_ds = Chart::OFC::Dataset::Line->new(
        values => \@user_count,
        label  => 'Users',
        color  => '#6aa41d',
    );

    my $vendor_ds = Chart::OFC::Dataset::Line->new(
        values => \@vendor_count,
        label  => 'Entries',
        color  => '#0072B0',
    );

    my $chart = Chart::OFC::Grid->new(
