...
use VegGuide::Validate qw( validate validate_pos SCALAR_TYPE POS_INTEGER_TYPE );


my $UnitType = SCALAR_TYPE( regex => qr/^(?:mile|km)$/ );

my %EarthRadius = ( mile => 3956,
                    km   => 6371,
                  );
...

{
    ...
    sub lat_long_min_max
    {
        ...
        my ( $min_lat, $max_lat ) =
            map { rad2deg($_) } ( $lat - $lat_distance, $lat + $lat_distance );

        my $long_distance = $p{distance} / ( $radius * cos($lat) );
        my ( $min_long, $max_long ) =
            map { rad2deg($_) } ( $long - $long_distance, $long + $long_distance );
