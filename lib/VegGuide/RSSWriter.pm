...
sub new
{
    ...
    my $g = XML::Generator::RSS10->new( Handler => $w,
                                        modules => [ 'cc', 'dc', 'regveg' ],
                                        pretty  => 1,
                                      );

    my $client =
        VegGuide::Client->new_from_params( show_localized_content => 0,
                                           encoding               => 'utf8',
                                           show_utf8              => 1,
...
sub location_channel
{
    ...
    $self->{g}->channel
        ( title => "VegGuide.Org: " . $location->name,
          ...
          dc => { publisher => 'Compassionate Action for Animals',
...
sub site_channel
{
    ...
    $self->{g}->channel
        ( title => $title,
          link  => uri( scheme => 'http', host => VegGuide::Config->CanonicalWebHostname() ),
          description => $description,
          dc => { publisher => 'Compassionate Action for Animals',
                  rights    => "Copyright 2002 - $self->{current_year} "
                               . " Compassionate Action for Animals",
...
sub add_vendor_for_data_feed
{
    ...
    foreach my $f ( qw( phone address1 address2
...

    {
        ...
        {
            $rv{'long-description'} =
                $p{raw_wiki_text} ? $char_data : VegGuide::Util::text_to_html( text => $char_data );
        }
    }

    $rv{country} = $p{vendor}->location->country;

    my ( $average, $rating_count ) =
        $p{vendor}->weighted_rating_and_count;
    ...
    foreach my $f ( qw( allows_smoking accepts_reservations is_wheelchair_accessible ) )
    {
        (my $elt = $f) =~ s/_/-/g;
    ...
    $rv{'edit-link'} =
        entry_uri( vendor => $p{vendor}, path => 'edit_form', with_host => 1 );

    $rv{'edit-hours-link'} =
        entry_uri( vendor => $p{vendor}, path => 'edit_hours_form', with_host => 1 );

    $rv{'read-reviews-link'} = $self->_vendor_reviews_link($p{vendor});

    $rv{'write-review-link'} =
        entry_uri( vendor => $p{vendor}, path => 'review_form', with_host => 1 );
...
sub _vendor_basic_item
{
    ...
    if ( $expand_description )
    {
        ...
        {
            $description = VegGuide::Util::text_to_html( text => $vendor->long_description );
        }
        else
        {
            $description .= "<p>\n";
...
sub _vendor_reviews_link
{
    ...
    if ($user)
    {
        return entry_review_uri( vendor => $vendor, user => $user, with_host => 1 );
    }
    else
    {
        return entry_uri( vendor => $vendor, with_host => 1 );
...
sub add_review
{
    ...
    my $description = VegGuide::Util::text_to_html( text => $comment->comment );
    $description .= "\n<p>($rating / 5)</p>\n"
        if $rating;

    $self->{g}->item
        ( title => 'review of ' . $vendor->name . ', written by ' . $user->real_name,
          link  => $review_link,
          description => $description,
          dc =>
          { date =>
