...
sub All
{
    ...
    {
        @order_by = ( $schema->VendorComment_t()->last_modified_datetime_c(),
                      'DESC',
    ...
    return
        $class->cursor
            ( $schema->join
                  ( select   => [ $schema->tables( 'VendorComment', 'Vendor', 'User' ) ],
                    join     => [ [ $schema->tables( 'VendorComment', 'Vendor' ) ],
...
sub Count
{
    ...
    return
        $schema->VendorComment_t()->function
            ( select =>
