...
use VegGuide::AlzaboWrapper (
    table => VegGuide::Schema->Schema()->Attribute_t() );
...
BEGIN {
    %Attributes = (
        map {
            $_->select('attribute_id') =>
                VegGuide::Attribute->SUPER::new( object => $_ )
            } VegGuide::Schema->Connect()->Attribute_t()->all_rows()
            ->all_rows()
    );
...
sub _new_row {
    ...
    if ( $p{name} ) {
        my @where;
        push @where, [ $schema->Attribute_t->name_c, '=', $p{name} ];
